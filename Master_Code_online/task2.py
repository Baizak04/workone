...
# print(next(x))
# print(next(x))
# range() это не итератор, а итерируемый объект. На месте х должно быть iter(range(2))

# 32)
# raise SystemExit
# SystemExit вызывается, когда программа завершает свою работу раньше времени (с помощью exit(), например), поэтому приложение просто останавливается и ничего не выводит.  

# 33)
print('abc'* 2,
      'abc' * 0,
      'abc' * -2)
# Умножение последовательности (список строка), приводит к дублированию элементов х раз. При умножении на число меньше или равное нулю, ответ будет ‘ ’.

# 34)
# x = b'abc'
# x[0] = b'0'
# print(x)
# Объект bytes() - неизменяемый. Для Этого его нужно обернуть в bytearray().  
