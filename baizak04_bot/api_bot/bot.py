...
@bot.message_handler(commands=['bad_habits'])
def bad_habits_txt(message):
    text = open('api_bot/txt/bad_habits.txt', 'r', encoding="utf8").read()
    for mess in util.smart_split(text, 3000):
        bot.send_message(message.chat.id, mess )
...
def get_user_text(message):
    ...
    elif message.text.lower() == "что у тебя на уме?":
        sleep(4)
    ...
    elif message.text.lower() == "как поживаешь?":
        sleep(6)
    ...
    elif message.text.lower() == "ты натурал или нет?":
        sleep(4)
    ...
    elif message.text.lower() == "что сегодня будешь делать?":
        sleep(1)
        bot.send_message(message.chat.id, 'Я буду учиться развиваться и помогать вам 🤗')
    elif message.text.lower() == "когда спать?":
        sleep(1)
        bot.send_message(message.chat.id, 'Я не буду спать \n я буду охранят вас до утра')
    elif message.text.lower() == "где ты родилась?":
        sleep(1)
        bot.send_message(message.chat.id, 'Я родилась в стране Кыргызстан 🇰🇬 город Бишкек')
    elif message.text.lower() == "кто ты по жизни?":
        sleep(1)
        bot.send_message(message.chat.id, 'Я просто человек, который прекрасно знает свое место в мире и стремится к большему 😎 .')
    elif message.text.lower() == "почему я люблю свой девушку?":
        sleep(5)
        bot.send_message(message.chat.id, '🤖 \n каждая пара имеет уникальные аспекты и особенности, которые влияют на их любовь друг к другу. Вы можете проанализировать свои чувства и отношение к своей девушке, чтобы определить, что именно привлекает вас и заставляет любить ее. ❤️')
